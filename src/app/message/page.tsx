  ...
  const CustomControl = ({ children, ...props }: any) => {
    return (
      <components.Control {...props}>
        <img src="/arealocation.svg" alt="Location" className="w-5 h-5 mr-1 ml-1" /> {/* Icon added here */}
        {children}
      </components.Control>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-[#939cab] pb-20 mb-3"> {/* pb-20 for fixed nav bar */}
      <main className="p-4">
        ...
        {/* Constituency Filter */}
        <div className="mb-4">
          <label htmlFor="constituency-filter " className="sr-only postsection-dropdown-text">सभी निर्वाचन क्षेत्</label>
          ...
          <div className="relative">
            ...
            <Select
              ...
              styles={{
                control: (provided) => ({
                  ...provided,
                  paddingLeft: '', // Adjust for icon - now handled by CustomControl
                  borderRadius: '0.5rem',
                  borderColor: '#e5e7eb',
                  boxShadow: 'none',
                  '&:hover': {
                ...
                placeholder: (provided) => ({
                  ...provided,
                  color: '#6b7280', // Styled placeholder text color
                  paddingLeft: '0.25rem', // Space between icon and text
                  fontWeight: 'medium', // Example: make placeholder bolder
                }),
                singleValue: (provided) => ({
                  ...provided,
                  color: '#1f2937',
                  paddingLeft: '0.25rem', // Space between icon and selected value
                }),
        ...
        {/* Posts List */}
        <div className={`space-y-4 ${!hasNextPage ? 'mb-10' : 'mb-4'}`}>
          {posts.length === 0 ? (
            ...
            posts.map(post => (
              <div key={post._id} className="bg-white rounded-lg shadow-sm p-4">
                ...
                {post.content && <p className=" mb-3 postsection-post-content-text">{post.content}</p>}
                ...
                <div className="mt-4 pt-4 space-y-3">
                  ...
                  <div className="flex flex-col items-center space-x-2 w-full gap-2">
                    <textarea // This is your input area
                      ...
                      className={`flex-1 p-2 border rounded-md w-full h-[120px] text-sm focus:outline-none focus:ring-1 focus:ring-[#273F4F] ${errorCommentInput.get(post._id) ? 'border-red-500 shake-effect' : 'border-gray-200'}`}
      ...
      <button onClick={() => router.push('/newPost')} className="bg-[#273F4F] text-white px-4 py-2 rounded-lg flex items-center justify-center z-10 shadow-lg fixed bottom-20 right-4 gap-2">
        <span>+</span> <span>नई चर्चा</span>
      </button>
    </div>
  )
}
